...
function isFocusable(element) {
  const window = utilities.getWindow(element);
  ...
  constructor(manager, options) {
    ...
    const cleanupEffects = state.effects(() => {
      for (const draggable of manager.registry.draggables.value) {
        ...
        const window = utilities.getWindow(activator);
        if (activator) {
...
var _Feedback = class _Feedback extends abstract.Plugin {
  constructor(manager, options) {
    super(manager);
    let style;
    let initialSize = void 0;
    ...
    const cleanupEffect = state.effect(() => {
      ...
      let cleanup;
      const shape = new utilities.DOMRectangle(element, { ignoreTransforms: true });
      ...
      if (!transformOrigin) {
        const { x, y } = state.untracked(() => position.current);
        transformOrigin = {
          x: (x - left) / width,
          y: (y - top) / height
        };
      ...
      const actual = new utilities.DOMRectangle(element, { ignoreTransforms: true });
      const offset = {
      ...
      dropEffectCleanup = state.effect(function dropAnimation() {
        if (dragOperation.status.dropped) {
          ...
          manager.renderer.rendering.then(() => {
            ...
            const final = new utilities.DOMRectangle(target);
            const current = new utilities.DOMRectangle(element);
            const delta2 = {
...
function createPlaceholder(source) {
  return state.untracked(() => {
    ...
    for (const [droppable, identifierAttribute] of containedDroppables) {
      ...
      utilities.ProxiedElements.set(current, clonedElement);
      cleanup.push(() => {
        utilities.ProxiedElements.delete(current);
...
var _PointerSensor = class _PointerSensor extends abstract.Sensor {
  ...
  handlePointerDown(event, source, options = {}) {
    if (this.disabled || !event.isPrimary || event.button !== 0 || !utilities.isElement(event.target) || source.disabled) {
      return;
    }
    const { target } = event;
  ...
  sensors: [
    PointerSensor.configure({
      activationConstraints(event, source) {
        ...
        if (pointerType === "mouse" && utilities.isElement(target) && (source.handle === target || ((_a3 = source.handle) == null ? void 0 : _a3.contains(target)))) {
          return void 0;
        }
        return {
      ...
      __spreadProps(__spreadValues({}, input), {
        ...
        effects: () => [
          ...
          () => {
            ...
            const observePosition = source && dragOperation.status.initialized && element2 && !this.disabled && this.accepts(source);
            if (observePosition) {
